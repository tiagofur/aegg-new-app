...
1. Construir modulo `features/clientes` con rutas, lista y formulario reutilizando componentes de forms existentes.
2. Crear hook `useClienteSearch` con debounce para autocompletar; exponer componente `ClienteSelector` reusable.
3. Actualizar flujo de creacion/edicion de trabajo para usar `ClienteSelector` y asignacion de miembro.
