...
export const reportesMensualesService = {
    async importar(importDto: ImportReporteMensualDto): Promise<ReporteMensual> {
        ...
        const { data } = await api.post('/reportes-mensuales/importar', formData, {
            headers: {
    ...
    async procesarYGuardar(mesId: string): Promise<{ success: boolean; message: string }> {
        const { data } = await api.post(`/reportes-mensuales/${mesId}/procesar`)
        return data
    },

    async obtenerDatos(mesId: string, reporteId: string): Promise<{ datos: any[][] }> {
        const { data } = await api.get(`/reportes-mensuales/${mesId}/${reporteId}/datos`)
        return data
    },
    ...
    async actualizarDatos(
        ...
        const { data } = await api.put(`/reportes-mensuales/${mesId}/${reporteId}/datos`, { datos })
        return data
    },
    ...
    async limpiarDatos(
        ...
        const { data } = await api.delete(`/reportes-mensuales/${mesId}/${reporteId}/datos`)
        return data
    },
}
