...
export const MesCard: React.FC<MesCardProps> = ({
    ...
    const isReadOnly = mes.estadoRevision === 'ENVIADO' || mes.estadoRevision === 'APROBADO'
    ...
    React.useEffect(() => {
        console.log('🔍 MesCard DEBUG:', {
            ...
            isAdmin,
            esGestor,
            deberianMostrarseLosBotones: puedeRevisar && mes.estadoRevision === 'ENVIADO',
        })
    ...
    const revisionBadge = useMemo(() => {
        switch (mes.estadoRevision) {
            ...
            default:
                return {
                    label: mes.estado === 'COMPLETADO' ? 'Listo para enviar' : 'En edición',
    ...
    const handleReabrirMes = async () => {
        ...
        if (!puedeRevisar) {
            alert('Solo el gestor responsable o un administrador puede reabrir el mes.')
            return
        }
    ...
    const handleEnviarRevision = async () => {
        ...
        const confirmar = window.confirm(
            `¿Deseas enviar el mes de ${MESES_NOMBRES[mes.mes - 1]} a revisión del gestor?`
        )
        if (!confirmar) return
        ...
        try {
            ...
            await mesesService.enviarRevision(mes.id, comentario?.trim() || undefined)
            alert('Mes enviado a revisión')
            onMesUpdated?.()
        } catch (error: any) {
            console.error('Error al enviar a revisión:', error)
            alert(error.response?.data?.message || 'Error al enviar el mes a revisión')
        } finally {
            setProcessing(false)
        }
    }
    ...
    const handleEnviarRevisionManual = async () => {
        if (!puedeRevisar) {
            alert('Solo el gestor responsable o un administrador puede enviar el mes manualmente.')
            return
        }
        ...
        <div className="border border-gray-300 rounded-lg overflow-hidden">
            ...
            {expanded && (
                <div className="p-3 bg-white">
                    <p className="text-sm font-semibold text-gray-700 mb-2">Reportes Mensuales:</p>
