...
import { trabajosService, reportesMensualesService, mesesService } from '../../services'
...
export const TrabajoDetail: React.FC<TrabajoDetailProps> = ({
    ...
    const [mesSeleccionado, setMesSeleccionado] = useState<string | undefined>(trabajo.meses[0]?.id)
    const [reporteSeleccionado, setReporteSeleccionado] = useState<string | undefined>()
    const [mostrarImportDialog, setMostrarImportDialog] = useState(false)
    const [mostrarImportReporteMensualDialog, setMostrarImportReporteMensualDialog] =
        useState(false)
    ...
    const canReabrir = canManage && isAprobado && (isAdmin || esGestorResponsable)
    ...
    useEffect(() => {
        ...
        if (mesIdParam && trabajo.meses) {
            ...
            if (mesEncontrado) {
                ...
                setTimeout(() => {
                    const mesElement = document.getElementById(`mes-card-${mesEncontrado.id}`)
                    if (mesElement) {
    ...
    const mesToneClasses = mesEstadoVisual ? MES_ESTADO_TONE_CLASSES[mesEstadoVisual.tone] : null
    const mesSeleccionadoLabel = mesActual ? MESES_NOMBRES[mesActual.mes - 1] : 'Seleccionar mes'

    const mesEstaBloqueado = Boolean(
        mesActual &&
        (mesActual.estadoRevision === 'ENVIADO' || mesActual.estadoRevision === 'APROBADO')
    )
    ...
    const puedeGestionarReportesMensuales = Boolean(canManageReportesMensuales && !mesEstaBloqueado)

    // Encontrar el reporte seleccionado
    const reporteActual = mesActual?.reportes?.find((r) => r.id === reporteSeleccionado)
    ...
    const aprobadorNombre = trabajo.aprobadoPor?.nombre ?? trabajo.aprobadoPor?.name ?? ''
    ...
    const handleLimpiarDatos = async () => {
        ...
        try {
            await reportesMensualesService.limpiarDatos(mesActual.id, reporteActual.id)
            ...
            alert(error.response?.data?.message || 'Error al limpiar los datos del reporte')
        }
    }
    ...
    const handleEnviarMesManual = async () => {
        ...
        const confirmar = window.confirm(
            `¿Deseas enviar el mes de ${MESES_NOMBRES[mesActual.mes - 1]} a revisión del gestor?`
        )
        ...
        try {
            ...
            alert(error?.response?.data?.message || 'No fue posible enviar el mes a revisión.')
        } finally {
        ...
        <div className="px-2 py-3">
            ...
            <div className="mb-3">
                ...
                <nav className="flex items-center gap-2 text-sm text-gray-500 mb-3">
                    ...
                    <span>/</span>
                    <span className="text-gray-900 font-medium">{trabajo.clienteNombre}</span>
                </nav>
            ...
            <ReporteAnualHeader
                ...
                onVerReporteBase={() => navigate(`/trabajos/${trabajo.id}/reporte-base-anual`)}
                onImportarExcel={() => {
                    if (!canEdit) {
                        return
                    }
                ...
                onDescargarExcel={() => alert('Funcionalidad de descarga en desarrollo')}
