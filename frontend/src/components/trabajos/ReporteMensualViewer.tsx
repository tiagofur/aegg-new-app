...
import { ReporteMensual, TIPOS_REPORTE_NOMBRES, EstadoRevisionMes } from '../../types/trabajo'
...
export const ReporteMensualViewer: React.FC<ReporteMensualViewerProps> = ({
    ...
    const esGestorResponsable = gestorResponsableId ? gestorResponsableId === userId : esGestor
    const puedeRevisar = isAdmin || esGestorResponsable
    const isMiembro = role === 'Miembro'
    const isReadOnly = mesEstadoRevision === 'ENVIADO' || mesEstadoRevision === 'APROBADO'
    const deberianMostrarseLosBotones = puedeRevisar && mesEstadoRevision === 'ENVIADO'
    ...
    const [guardarEnBaseContext, setGuardarEnBaseContext] = useState<GuardarEnBaseContext | null>(
        null
    )
    const [miAdminResumen, setMiAdminResumen] = useState<MiAdminIngresosTotales | null>(null)
    const [miAdminAutomationActions, setMiAdminAutomationActions] =
        useState<MiAdminAutomationActions | null>(null)
    const [auxiliarResumen, setAuxiliarResumen] = useState<AuxiliarResumenState | null>(null)
    ...
    const esReporteComparable =
        reporte.tipo === 'INGRESOS_AUXILIAR' || reporte.tipo === 'INGRESOS_MI_ADMIN'
    const usaTablaEspecializada = reporte.tipo === 'INGRESOS' || esReporteComparable
    ...
    const reporteComplementario = useMemo(() => {
        ...
        const targetTipo =
            reporte.tipo === 'INGRESOS_AUXILIAR' ? 'INGRESOS_MI_ADMIN' : 'INGRESOS_AUXILIAR'
    ...
    const resultadoComparacion = useMemo<ComparacionResultado>(() => {
        if (!comparacionActiva || !reporteComplementario || !tieneDatos || usaTablaEspecializada) {
            return {
        ...
        const foliosComplementarios = new Map<string, { valores: unknown[]; subtotal: number }>()
            ...
            if (colSubtotalComp >= 0) {
                ...
                if (!Number.isNaN(bruto)) {
                    ...
                    if (colMonedaComp >= 0 && colTipoCambioComp >= 0) {
                        ...
                        const tipoCambio = Number.parseFloat(String(fila[colTipoCambioComp] ?? 1))
        ...
        for (let i = 1; i < datosActuales.length; i++) {
            ...
            if (colMonedaActual >= 0 && colTipoCambioActual >= 0) {
                ...
                const tipoCambio = Number.parseFloat(String(fila[colTipoCambioActual] ?? 1))
                if (!Number.isNaN(tipoCambio) && moneda !== 'MXN') {
                    subtotal = subtotal * (tipoCambio || 1)
                }
            }
    ...
    }, [comparacionActiva, reporteComplementario, reporte.datos, tieneDatos, usaTablaEspecializada])
    ...
    const ultimaActualizacion = formatFecha(reporte.fechaImportacion || reporte.fechaCreacion)

    const comparacionDisponible = Boolean(esReporteComparable && reporteComplementario)
    useEffect(() => {
    ...
    const handleGuardarEnBaseContextChange = useCallback((context: GuardarEnBaseContext | null) => {
    ...
    const handleAuxiliarResumenChange = useCallback((resumen: AuxiliarResumenState | null) => {
    ...
    const handleAprobarMes = async () => {
        if (!puedeRevisar) {
            alert('Solo el gestor responsable o un administrador puede aprobar el mes.')
            return
        }
    ...
    const handleSolicitarCambios = async () => {
        if (!puedeRevisar) {
            alert('Solo el gestor responsable o un administrador puede solicitar cambios.')
            return
        }
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                                            ...
                                            <tbody className="divide-y divide-gray-100 bg-white">
                                                {filas.map((fila, rowIndex) => (
                                                    <tr
                                                        key={rowIndex}
                                                        className={getRowClassName(fila)}
                                                    >
                                            ...
                                            {comparacionActiva && (
                                                <>
                                                    ...
                                                    <span className="font-medium text-yellow-700">
                                                        {resultadoComparacion.foliosUnicos.size}{' '}
                                                        únicos
                                                    </span>
                                                    {' • '}
                                        ...
                                        <div className="mb-4 flex items-start gap-4">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="mb-4 text-sm text-gray-600">
                                                    Esta acción eliminará todos los datos importados
                                                    del reporte <strong>{nombre}</strong>. El
                                                    reporte volverá al estado "Sin importar".
                                                </p>
