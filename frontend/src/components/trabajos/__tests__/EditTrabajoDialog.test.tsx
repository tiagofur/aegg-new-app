...
function clientesMockFactory() {
    const ClienteSelector = React.forwardRef(
        (
            ...
            return (
                <div>
                    <button type="button" onClick={() => onChange?.(mockCliente.id, mockCliente)}>
...
describe('EditTrabajoDialog', () => {
    ...
    it('abre el modal en modo creación y refresca el selector tras guardar', async () => {
        ...
        render(
            <EditTrabajoDialog trabajo={baseTrabajo} isOpen onClose={vi.fn()} onSuccess={vi.fn()} />
        )

        await user.click(screen.getByRole('button', { name: /crear nuevo cliente/i }))
    ...
    it('abre el modal en modo edición cuando hay cliente seleccionado', async () => {
        ...
        await user.click(screen.getByRole('button', { name: /editar cliente seleccionado/i }))
