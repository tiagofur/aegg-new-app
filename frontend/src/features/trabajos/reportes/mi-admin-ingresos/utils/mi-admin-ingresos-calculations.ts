...
export const parseExcelToMiAdminIngresos = (
    ...
    const subtotalAuxIndex = findColumnIndex(normalized, [
        'subtotalaux',
        'subtotal aux',
        'subtotal auxiliar',
    ])
    const subtotalMxnIndex = findColumnIndex(normalized, ['subtotalmxn', 'subtotal mxn'])
    const tcSugeridoIndex = findColumnIndex(normalized, [
        'tcsugerido',
        'tc sugerido',
        'tc_sugerido',
    ])
    ...
    if (estadoIndex === -1) {
        console.warn(
            '⚠️ Columna "Estado SAT" no encontrada en Mi Admin. Se agregará automáticamente con valor "Vigente".'
        )
    ...
    if (auxiliarData) {
        ...
        console.log(
            `📚 ${auxiliarLookup.size} registros de Auxiliar disponibles para lookup por FOLIO`
        )
        console.log(
            `📚 Primeros FOLIOs en Auxiliar:`,
            Array.from(auxiliarLookup.keys()).slice(0, 5)
        )
    }
    ...
    for (let i = dataStartRow; i < excelData.length; i++) {
        ...
        if (i < dataStartRow + 5) {
            // Loguear solo las primeras 5 filas de datos
 ...
 */
export const calculateTotales = (data: MiAdminIngresosRow[]): MiAdminIngresosTotales => {
