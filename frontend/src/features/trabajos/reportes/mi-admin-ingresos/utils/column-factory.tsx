...
import { TCSugeridoCell, EditableTipoCambioCell, EditableEstadoSatCell } from '../components/cells'
    ...
    return {
        ...
        subtotalAUX: {
            ...
            cell: (info: SpecialCellContext) => {
                ...
                if (value === null || Number.isNaN(value)) {
                    return <span className="text-gray-400 text-sm text-center block">N/A</span>
                }
                return (
        ...
        tcSugerido: {
            ...
            cell: (info: SpecialCellContext) => {
                const row = info.row.original
                if (row.isSummary) {
                    return <span className="text-center block text-sm text-gray-500">-</span>
                }
                const value = info.getValue() as number | null
                return (
                    <TCSugeridoCell
...
export function createMiAdminDynamicColumns(
    ...
    return columnKeys.map((key) => {
        ...
        return columnHelper.accessor((row) => row[key as keyof MiAdminIngresosRow] as unknown, {
            id: key,
