...
import { recalculateRowAfterTipoCambioChange, updateRowEstadoSat } from '../utils'
...
export const useMiAdminIngresosEdit = ({ data, auxiliarData }: UseMiAdminIngresosEditProps) => {
    // Map de cambios: folio -> cambios parciales
    const [edits, setEdits] = useState<Map<string, Partial<MiAdminIngresosRow>>>(new Map())
    ...
    const cancelarFoliosUnicos = useCallback(() => {
        ...
        const auxiliarFolios = new Set(
            auxiliarData.filter((row) => row.estadoSat === 'Vigente').map((row) => row.folio)
        )
