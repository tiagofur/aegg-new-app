...
export const useMiAdminIngresosCalculations = ({
    ...
    const dataWithTotals = useMemo(() => {
        ...
        const ivaTotal = data.reduce((sum, row) => (
            row.estadoSat === 'Vigente' ? sum + (row.iva || 0) : sum
        ), 0);

        const totalTotal = data.reduce((sum, row) => (
            row.estadoSat === 'Vigente' ? sum + (row.total || 0) : sum
        ), 0);
