 ...
 */
export function inferColumnType(columnName: string, sampleValues: any[]): 'currency' | 'date' | 'number' | 'text' {
    const normalizedName = columnName.toLowerCase();

    // Verificar por nombre
    if (CURRENCY_FIELDS.has(columnName) || normalizedName.includes('subtotal') || normalizedName.includes('total') || normalizedName.includes('iva')) {
        return 'currency';
    }
    if (DATE_FIELDS.has(columnName) || normalizedName.includes('fecha')) {
        return 'date';
    }
    if (NUMERIC_FIELDS.has(columnName) || normalizedName.includes('tipo') || normalizedName.includes('cambio')) {
        return 'number';
    }
