...
export const GuardarEnBaseButton: React.FC<GuardarEnBaseButtonProps> = ({
    ...
    const diferencia = totalsAvailable ? Math.abs(totalMiAdmin - (totalAuxiliar ?? 0)) : null
    ...
    const error = errorResumen || errorExcelMutation || (errorExcel ? new Error(errorExcel) : null)
    ...
    const getTooltip = (): string => {
        if (!hasAuxiliarData) return 'Importa y procesa el Auxiliar para habilitar el guardado'
        if (totalAuxiliar === null) return 'Calculando totales auxiliares, intenta de nuevo'
