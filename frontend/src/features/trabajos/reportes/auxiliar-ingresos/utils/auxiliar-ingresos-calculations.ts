 ...
 */
export const parseExcelToAuxiliarIngresos = (excelData: any[][]): AuxiliarIngresosRow[] => {
    ...
    if (estadoIndex === -1) {
        console.warn(
            '⚠️ Columna "Estado SAT" no encontrada en Auxiliar. Se agregará automáticamente.'
        )
    ...
    for (let i = dataStartRow; i < excelData.length; i++) {
        ...
        const razonSocial =
            razonSocialIndex !== -1 ? row[razonSocialIndex]?.toString().trim() || null : null
        ...
        console.log(
            `🔍 Row ${i}: FOLIO = "${folio}", Estado SAT = "${estadoSat}" (raw: "${estadoRaw}")`
        )
 ...
 */
export const calculateTotales = (data: AuxiliarIngresosRow[]): AuxiliarIngresosTotales => {
    ...
    const porcentajeVigentes = cantidadTotal > 0 ? (vigentes.length / cantidadTotal) * 100 : 0
    const porcentajeCanceladas = cantidadTotal > 0 ? (canceladas.length / cantidadTotal) * 100 : 0
    const promedioSubtotalVigentes = vigentes.length > 0 ? totalSubtotal / vigentes.length : 0
