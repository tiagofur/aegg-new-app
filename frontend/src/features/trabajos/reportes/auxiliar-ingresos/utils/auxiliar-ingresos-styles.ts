...
import { AuxiliarIngresosRow, ComparisonResult, ComparisonStatus } from '../types'
...
export const footerStyles = {
    ...
    comparisonMatch:
        'sticky bottom-0 bg-blue-50 text-blue-900 font-bold border-t-2 border-blue-400',

    // Con comparación - totales no coinciden
    comparisonMismatch:
        'sticky bottom-0 bg-red-50 text-red-900 font-bold border-t-2 border-red-400',
} as const
 ...
 * @returns String con clases CSS de Tailwind
 */
export const getFooterBackgroundColor = (totalesMatch: boolean | null): string => {
    if (totalesMatch === null) {
        return footerStyles.base
    }

    return totalesMatch ? footerStyles.comparisonMatch : footerStyles.comparisonMismatch
}
 ...
 */
export const getComparisonTooltipMessage = (comparison: ComparisonResult): string => {
 ...
 * @returns Clases CSS
 */
export const getEstadoSatCellClasses = (estadoSat: 'Vigente' | 'Cancelada'): string => {
