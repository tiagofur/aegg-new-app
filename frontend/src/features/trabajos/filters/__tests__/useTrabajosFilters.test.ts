...
describe("useTrabajosFilters", () => {
    ...
    it("lee los filtros desde localStorage al iniciar", () => {
        const stored = JSON.stringify({ year: 2024, clienteId: "cliente-1", search: "demo", estado: "EN_REVISION" });
        window.localStorage.setItem(STORAGE_KEY, stored);

        const { result } = renderHook(() => useTrabajosFilters());

        expect(result.current.filters).toEqual({ year: 2024, clienteId: "cliente-1", search: "demo", estado: "EN_REVISION" });
    });
    ...
    it("restablece filtros a valores por defecto", async () => {
        ...
        expect(result.current.filters).toEqual({ year: undefined, clienteId: undefined, estado: undefined, search: "" });
    });
